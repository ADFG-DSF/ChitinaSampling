...
for(iyear in seq_along(years)) {
  sockeye[[iyear]] <- read.csv(file=paste0(years[iyear], " Raw Data - Sockeye.csv")) %>% 
    clean_names %>% remove_empty(which="cols")
  chinook[[iyear]] <- read.csv(file=paste0(years[iyear], " Raw Data - Chinook.csv")) %>% 
    clean_names %>% remove_empty(which="cols")
  GSD[[iyear]] <- read.csv(file=paste0(years[iyear], " GSD HARVEST Reported.csv")) %>% 
    clean_names %>% remove_empty(which="cols")
  CSD[[iyear]] <- read.csv(file=paste0(years[iyear], " CSD HARVEST Reported.csv")) %>% 
    clean_names %>% remove_empty(which="cols")
}
...
# should plot this, I think it would be cool
...
# finding quartiles to assign strata
...
fixages <- function(x) {
  ...
  # x1$age <- paste(as.character(x1$age), sep=".",collapse="")
  x1$totage <- as.numeric(substr(x1$age,1,1)) + as.numeric(substr(x1$age,nchar(x1$age),nchar(x1$age)))
  return(x1)
}
fixsex <- function(x) {
  x1 <- x[x$sex %in% c("F","M"), ]
...
stratprop <- function(cat, strat, Nstrat) { # membership vec, strata vec, strata weights
  ntz <- table(strat, cat)
  nz <- colSums(ntz)
  nt <- rowSums(ntz)
  ptz <- ntz/nt
  ...
  pz <- Nz/sum(Nt)
  vpz <- vNz/(sum(Nt)^2)
  fortab <- paste0(round(100*pz, 2), "% (", round(100*sqrt(vpz), 2), "%)")
  harv <- paste0(round(Nz, 0), " (", round(sqrt(vNz), 0), ")")
  data.frame(nz,pz,vpz,fortab,harv)
}
...
for(i in 1:4) {
  ...
  print(rbind(aa12a, aa3, aa4))   ### this will become knitr::kable
...
### then figure out how to test &/or combine what was done for Chinook - pooled by both subdistricts!
